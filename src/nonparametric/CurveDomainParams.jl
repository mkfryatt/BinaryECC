...
struct CurveDomainParams
    G::ECPointAffine #base point
    n::BigInt #order of G, ie nG = O
    h::BigInt #cofactor of n, #E(GF(2ᵐ)) / n
...
function isvalid(T::CurveDomainParams, t::Int)
    #1
    ...
    m = T.G.x.field.degree
    if !(m in [163, 233, 239, 283, 409, 571]) return false end
    if m<=2*t || m>=2*tprime return false end

    #2
    reductions = Dict([
        (163, [BigInt(128+64+8+1)]),
        (233, [(BigInt(1)<<74) + BigInt(1)]),
        (239, [(BigInt(1)<<36) + BigInt(1), (BigInt(1)<<158) + BigInt(1)]),
        (483, [(BigInt(1)<<12) + BigInt(128+32+1)]),
        (409, [(BigInt(1)<<87) + BigInt(1)]),
        (571, [(BigInt(1)<<10) + BigInt(32+4+1)])
    ])
    if !(T.G.x.field.reduction in reductions[m]) return false end
    ...
    if T.h != floor(Int, ((sqrt(2^m)+1)^2/T.n)) return false end
...
const SECT163K1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(FieldPoint(1, FIELD163), FieldPoint(1, FIELD163))
    ),
...
const SECT163R1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint("07 B6882CAA EFA84F95 54FF8428 BD88E246 D2782AE2", FIELD163),
            FieldPoint("07 13612DCD DCB40AAB 946BDA29 CA91F73A F958AFD9", FIELD163)
        )
    ),
...
const SECT163R2 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(1, FIELD163),
            FieldPoint("02 0A601907 B8C953CA 1481EB10 512F7874 4A3205FD", FIELD163)
        )
    ),
...
const SECT233K1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(0, FIELD233),
            FieldPoint(1, FIELD233)
        )
    ),
...
const SECT233R1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(1, FIELD233),
            FieldPoint("0066 647EDE6C 332C7F8C 0923BB58 213B333B 20E9CE42 81FE115F 7D8F90AD", FIELD233)
        )
    ),
...
const SECT239K1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(0, FIELD239B),
            FieldPoint(1, FIELD239B)
        )
    ),
...
const SECT283K1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(0, FIELD283),
            FieldPoint(1, FIELD283)
        )
    ),
...
const SECT283R1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(1, FIELD283),
            FieldPoint("027B680A C8B8596D A5A4AF8A 19A0303F CA97FD76 45309FA2 A581485A F6263E31 3B79A2F5", FIELD283)
        )
    ),
...
const SECT409K1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(0, FIELD409),
            FieldPoint(1, FIELD409)
        )
    ),
...
const SECT409R1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(1, FIELD409),
            FieldPoint("0021A5C2 C8EE9FEB 5C4B9A75 3B7B476B 7FD6422E F1F3DD67 4761FA99 D6AC27C8 A9A197B2 72822F6C D57A55AA 4F50AE31 7B13545F", FIELD409)
        )
    ),
...
const SECT571K1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(0, FIELD571),
            FieldPoint(1, FIELD571)
        )
    ),
...
const SECT571R1 = CurveDomainParams(
    ECPointAffine(
        ...
        EC(
            FieldPoint(1, FIELD571),
            FieldPoint("02F40E7E 2221F295 DE297117 B7F3D62F 5C6A97FF CB8CEFF1 CD6BA8CE 4A9A18AD 84FFABBD 8EFA5933 2BE7AD67 56A66E29 4AFD185A 78FF12AA 520E4DE7 39BACA0C 7FFEFF7F 2955727A", FIELD571)
        )
    ),
