...
"""
struct CurveDomainParams{D,R,T}
    G::ECPointAffine{D,R,T} #generator point
    n::BigInt #order of G, ie nG = O
    h::BigInt #cofactor
    CurveDomainParams(G::ECPointAffine{D,R,T}, n::Integer, h::Integer) where {D,R,T} =
        new{D,R,T}(G, convert(BigInt, n), convert(BigInt, h))
end

#sec1 v2, 3.1.2.2.1
#Elliptic Curve Domain Parameters over F_{2^m} Validation Primitive
"""
    isvalid(T::CurveDomainParams{D,R,T}, t::Int) where {D,R,T}
...
"""
function isvalid(T::CurveDomainParams{D,R,T2}, t::Int) where {D,R,T2}
    #1
...
SECT163K1(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(BFieldPoint163{T}(1), BFieldPoint163{T}(1)))
    ),
...
SECT163R1(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(
            BFieldPoint163{T}("07 B6882CAA EFA84F95 54FF8428 BD88E246 D2782AE2"),
            BFieldPoint163{T}("07 13612DCD DCB40AAB 946BDA29 CA91F73A F958AFD9")
        ))
    ),
...
SECT163R2(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(
            BFieldPoint163{T}(1),
            BFieldPoint163{T}("02 0A601907 B8C953CA 1481EB10 512F7874 4A3205FD")
        ))
    ),
...
SECT233R1(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(
            BFieldPoint233{T}(1),
            BFieldPoint233{T}("0066 647EDE6C 332C7F8C 0923BB58 213B333B 20E9CE42 81FE115F 7D8F90AD")
        ))
    ),
...
SECT283R1(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(
            BFieldPoint283{T}(1),
            BFieldPoint283{T}("027B680A C8B8596D A5A4AF8A 19A0303F CA97FD76 45309FA2 A581485A F6263E31 3B79A2F5")
        ))
    ),
...
SECT409R1(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(
            BFieldPoint409{T}(1),
            BFieldPoint409{T}("0021A5C2 C8EE9FEB 5C4B9A75 3B7B476B 7FD6422E F1F3DD67 4761FA99 D6AC27C8 A9A197B2 72822F6C D57A55AA 4F50AE31 7B13545F")
        ))
    ),
...
SECT571R1(T::Type{U}) where U<:Unsigned = CurveDomainParams(
    ECPointAffine(
        ...
        Ref(EC(
            BFieldPoint571{T}(1),
            BFieldPoint571{T}("02F40E7E 2221F295 DE297117 B7F3D62F 5C6A97FF CB8CEFF1 CD6BA8CE 4A9A18AD 84FFABBD 8EFA5933 2BE7AD67 56A66E29 4AFD185A 78FF12AA 520E4DE7 39BACA0C 7FFEFF7F 2955727A")
        ))
    ),
