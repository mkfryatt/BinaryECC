...
struct StaticUInt{L,T<:Unsigned}
    value::Vector{T}
    StaticUInt{L,T}(value::Vector{T}) where {L,T} = new(value)
end
...
function StaticUInt{L,T}(value::Integer)::StaticUInt{L,T} where {L,T}
    ...
    valuevector::Vector{T} = [T(0) for i=1:L]
    bitmask = typemax(T)
    for i in 1:L
...
function isone(x::StaticUInt{L,T})::Bool where {L,T}
    return iszero(x.value[2:L]) && isone(x.value[1])
end
...
function zero(::Type{StaticUInt{L,T}})::StaticUInt{L,T} where {L,T}
    return StaticUInt{L,T}([T(0) for i=1:L])
end
...
function random(::Type{StaticUInt{L,T}})::StaticUInt{L,T} where {L,T}
    #value::Vector{T} = [rand(T) for i=1:L]
    return StaticUInt{L,T}([rand(T) for i=1:L])
end
